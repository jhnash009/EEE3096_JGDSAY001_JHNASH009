...
//array of 8-bit integers that holds binary values
...
int main(void)
{
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  ...
	  if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_RESET)  //checks if button PA0 is pressed
	  	{
		  //alternates between 1-second delay and half-second delay
		  if (TIM16 ->ARR == 1000 - 1){
			  TIM16 ->ARR = 500 - 1;}
		  else {
			  TIM16 ->ARR = 1000 - 1;
...
void TIM16_IRQHandler(void)
{
	...
	// TODO: Change to next LED pattern; output 0x01 if the read SPI data is incorrect
		...
		pVal = read_from_address(adrs);  //reads 8-bit value from EEPROM
		if (pVal == patterns[adrs]){     //checks whether value read corresponds to correct value in array
			GPIOB->ODR = 0b00000000;
			GPIOB->ODR =  pVal;          //write returned binary value to LEDs
			if (adrs < 5) {
			adrs++;
			}
			else {
			...
			GPIOB->ODR = 0b00000001;     //output 0x01 to LEDs to indicate SPI failure
